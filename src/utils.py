...
def tokenize_answers(sequences, vocab):
    ...
    tokenized_sequences = []
    sequence_length = get_sequence_length(sequences)
    for sequence in sequences:
        tokenized_sequence = []
        for word in sequence:
            tokenized_sequence.append(vocab.getIndex(word))
        padding_size = sequence_length - len(tokenized_sequence)
...
def tokenize_questions(questions, vocab):
    ...
    '''
    new_questions = []
    sequence_length = get_sequence_length(questions)
    for i, question in enumerate(questions):
        new_question = []
        for word in question:
            new_question.append(vocab.getIndex(word))
        padding_size = sequence_length - len(new_question)
